...
Page({
  ...
  showPrivacy() {
    wx.showModal({ title: '数据如何使用', showCancel: false, content: '偏好、计划、行动、足迹和对话保存在本机；启用云能力时会同步到你的个人云端空间。照片按需上传，失败时保留本机副本并重试。\n\n此刻推荐只使用你确认的地区摘要；足迹位置可选择仅名称或精确位置。AI 推荐会接收状态、兴趣和行动摘要，聊天会接收近期对话及生活摘要，回顾会接收所选日期内的记录和文字。照片及足迹经纬度不发送给 AI。\n\n删除足迹会清理关联照片；删除全部数据后仅保留用于阻止旧设备恢复数据的云端重置版本，不保留生活内容。' })
  },
  showHelp() { wx.showModal({ title: '怎样使用风月为邻', showCancel: false, content: '此刻：按时间和状态选择行动；直接去做的事情会保留在首页，回来即可记录。\n风月：愿望是模糊想法，计划是长期体验，行动是现在能做的一件具体事。\n足迹：保存经历，也能补记过去的事情。未知时长不等于零，最后没有去做会单独统计。\n提醒只在行动页或返回此刻时出现，不是后台闹钟。' }) },
  clearChats() {
