...
Page({
  ...
  onLoad(options) {
    ...
    const action = existing ? { ...existing } : { id: '', name: draft ? (draft.name || '') : '', domainId, minutes: draft ? Math.max(5, Number(draft.minutes) || 30) : 30, energy: ['low','medium','high'], environments: ['any'], preparation: draft ? (draft.preparation || '') : '', source: draft ? 'ai' : 'user' }
