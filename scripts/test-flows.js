...
async function main() {
  ...
  await test('系统分类可用于 AI 草稿但不会成为新用户个人数据', () => {
    const state = repo.getState()
    const draft = ai.normalizeOrganizedDraft({ type: 'plan', name: '去成都住几天', domainId: 'travel', why: '换个地方生活', actions: [{ name: '看看合适的时间', minutes: 20 }] }, { domains: require('../miniprogram/data/domains').SYSTEM_DOMAINS })
    assert.equal(state.domains.length, 0)
    assert.equal(draft.domainId, 'travel')
    assert.equal(draft.actions[0].domainId, 'travel')
  })
  await test('确认计划草稿后同时保存用户保留的第一步', () => {
    const target = repo.saveOrganizedDraft({ type: 'plan', name: '学会做家常菜', domainId: 'daily', why: '想照顾好自己', actions: [{ name: '选一道想吃的菜', minutes: 10 }, { name: '准备食材', minutes: 20 }] })
    const state = repo.getState(); const plan = state.plans.find((item) => item.id === target.id)
    assert.equal(target.type, 'plan')
    assert.equal(plan.actionIds.length, 2)
    assert.equal(state.actions.length, 2)
    assert.ok(state.actions.every((item) => item.domainId === 'daily' && item.source === 'ai'))
  })
  await test('首页想法先展示草稿，用户确认后才写入行动库', async () => {
    const p = page('now'); p.onLoad()
    ai.organizeThought = async () => ({ draft: { type: 'action', name: '晚饭后散步', domainId: 'health', minutes: 20, preparation: '穿舒服的鞋', actions: [] } })
    p.onThoughtInput({ detail: { value: '今晚想出去走走' } })
    await p.organizeThought()
    assert.equal(repo.getState().actions.length, 0)
    assert.equal(p.data.thoughtDraft.domainName, '运动健康')
    p.saveThoughtDraft()
    assert.equal(repo.getState().actions[0].name, '晚饭后散步')
    assert.equal(repo.getState().domains.length, 0)
  })
